...
import { PortInUseError } from "@omnilog/core";
...
export function SetupPage() {
  ...
  const hasLocalServer = useApp((s) => s.hasLocalServer);
  ...
  useEffect(() => {
    if (!deviceName && platformUI.defaultDeviceName) {
  ...
  async function onFreePortAndRetry(port: number) {
    setQuick({ kind: "starting" });
    await platformUI.killPort?.(port);
  ...
  return (
    <div className="setup">
      <div className="setup-card">
        ...
        {hasLocalServer && <div className="divider"><span>or connect manually</span></div>}
